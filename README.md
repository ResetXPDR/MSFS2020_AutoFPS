...
- Expert Settings
  - Auto Method - FPS Sensitivity generally gives better results and hence is the default. Use FPS Tolerance if you experience stuttering issues. Use Auto TLOD if you are using an FPS cap and it interferes with setting a suitable target FPS.
    ...
    - Auto TLOD - functions similar to Auto OLOD by using an altitude schedule.
      ...
      - TLOD Base + - When enabled, an additional TLOD offset may be achievable with favourable performance conditions.
  ...
  - TLOD Min + - When enabled, additional TLOD Min may be achievable with favourable performance conditions.
    - The TLOD Min + seek process will automatically start when commencing a flight, regardless of your aircraft's position, and at the conclusion of a flight when on the ground and stopped.
    - This seek process can be manually restarted by pressing the Reset button, should flight conditions change such that the original TLOD Base + is no longer valid.
