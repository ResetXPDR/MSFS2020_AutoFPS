...
- Lossless Scaling (LS) FG, including the scaling muliplier used, is also detected and the correct LSFG multiplied FPS is displayed. The app will first try to use an LS profile with the specific name MSFS2020 to obtain these settings. If an MSFS2020 profile does not exist then the settings in the Default profile will be used. If you are using LSFG 1.1 in LS and AutoFPS is not showing a 2X mulitplier, temporarily select LSFG 2.3 in LS and select the 2X mulitplier, switch back to LSFG 1.1 then press the Reset button on AutoFPS.
